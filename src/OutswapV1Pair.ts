...
export function handleMint(event: MintEvent): void {
  ...
  let token0Amount = formatUnits(event.params.amount0, token0.decimals)
  let token1Amount = formatUnits(event.params.amount1, token1.decimals)
...
export function handleSwap(event: SwapEvent): void {
  ...
  let token0 = Token.load(pair.token0)!
  let token1 = Token.load(pair.token1)!
  let amount0In = formatUnits(event.params.amount0In, token0.decimals)
  let amount1In = formatUnits(event.params.amount1In, token1.decimals)
  let amount0Out = formatUnits(event.params.amount0Out, token0.decimals)
  let amount1Out = formatUnits(event.params.amount1Out, token1.decimals)
...
export function handleSync(event: SyncEvent): void {
  ...
  pair.reserve0 = formatUnits(event.params.reserve0, token0.decimals)
  pair.reserve1 = formatUnits(event.params.reserve1, token1.decimals)
